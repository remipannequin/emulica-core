...
#from SimPy.SimulationStep import Simulation, Process, Resource, Store, Monitor, PriorityQ, put, get, hold, passivate, request, reactivate, release
import simpy
...
class Module:
    ...
    def create_report_socket(self, multiple_observation = False):
        ...
        if multiple_observation == False:
            ...
            #print "create report socket", self.report_socket
    ...
    def current_time(self):
        if 'is_main' in  dir(self) and self.is_main:
            if self.sim is None:
    ...
    def get_module(self, name):
        ...
        if len(names) == 1:
            if self.is_model():
                ...
                raise EmulicaError(self,_("Invalid module name: {0}").format(name))#TODO more explicit error
        else:
...
class Product:
    ...
    def dispose(self):
        ...
        """
        if self.__active:
            #print (" disposing product {pid} at {time}").format(pid = self.pid, time = self.model.current_time())
    ...
    def assemble(self, component, actuator, key = None):
        ...
        self.composition_history.append((self.model.current_time(), actuator, component.pid))#record info about composed and composite
        if key == None :
    ...
    class ModuleProcess:
        ...
        def run(self, module):
            ...
            while True:
                ...
                now = self.env.now
                if request_cmd.when > now:
                    #print "hold during ", (request_cmd.when - now)
                    yield self.env.timeout(request_cmd.when - now)
                if request_cmd.what == DisposeAct.produce_keyword:
                    #print "processing dispose cmd"
                    #print module.properties['source'].lock.users
                    #print module.properties['source'].lock.queue
...
class PushObserver(Module):
    ...
    class ModuleProcess:
        ...
        def __wait_and_reactivate(self, delay):
            ...
            logger.info(_("t={now}: will reactivate observer at {delayed}").format(now = self.env.now, delayed = delayed))
            yield self.env.timeout(delay)
            self.reactivate_dates.remove(delayed)
            self.__reactivate_now()
        
        def __reactivate_now(self):
            logger.info(_("t={now}: reactivating obs (ev is {ev})").format(now = self.env.now, ev = self.__reactivate))
